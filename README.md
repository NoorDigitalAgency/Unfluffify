...
Opening Unfluffify on a supported page establishes the fixed mobile extraction target (412x960). Actual browser emulation is active only while a live side panel owns that tab, so marking and AI-submission visibility use the crawler posture without leaving an ownerless page vulnerable to physical-resize artifacts. Closing the last panel returns the page to native browser geometry behind an opaque transition guard while retaining the desired target and any marking decisions; annotations and marking input remain parked. Reopening restores and proves the retained target behind the same guard before interaction resumes. There is no general device or manual-scale control.

When AI selectors already exist for the current property, the popup also shows a separate `Preview in desktop mode` checkbox. That narrow silent-preview choice persists as the tab's desired mode, switches an owned page into desktop emulation, and keeps silent previewing available. Enabling marking serially restores mobile emulation before activating marking; a clean disable may return to desktop silent preview. Closing the panel suspends either desired mode to native posture without forgetting it. While a panel remains live, unexpected debugger detach self-heals the exact owned target.

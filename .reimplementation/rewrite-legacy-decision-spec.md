...
| I-33 | Recognition establishes the tab-session desire for fixed mobile emulation; the silent-only desktop target remains its sole exception. Actual browser emulation is owned by at least one live side-panel document. While owned, navigation, debugger detach, tab rebinding, and committed viewport changes reassert/refit the same complete target without an opposite-mode flash. Last-owner close paint-guards a transition to native browser posture, retains the desired target as suspended, hides geometry-bound annotations/listeners without discarding marking decisions, and first-owner reopen restores that target behind the guard. Ownerless recovery never attaches CDP; transient transition/ownership refusal retains and retries the last proven active or suspended target rather than acknowledging ambiguity. |
...
| N-03 | Make render-mode inspection a durable, tokenized background-owned session. It survives reload and MV3 worker recreation, is adopted by the new content document, waits for an acknowledgement that the inspection surface painted, and clears only for a matching success, failure, timeout, cancellation, navigation, unregister, or teardown outcome. Last-side-panel closure is an explicit cancellation boundary: the inspection must complete its token/generation-fenced fail-open JavaScript restore (and required reload) before I-33 may detach the panel-owned debugger posture. |

...
| INV-1.7 | **Session working draft vs backend baseline.** The Hub is the source of truth for saved state. The background retains the complete validated `/load` corpus; a current marking-session draft is a separate overlay. Save responses prove only remote commit and invalidate cached authority. A distinct successful `/load` complete-replaces the baseline atomically; load failure leaves recovery fenced rather than promoting any Save response or draft. | **CORRECTED** — authority is background-owned and Load-derived, not popup-local. |
...
| INV-5.7 | **Genuine hiding → no UI, truthful payload coverage.** `display:none`, `visibility:hidden/collapse`, `opacity:0`, `hidden`, sr-only/clip-rect off-canvas, zero-area, and interaction-gated collapsed panels are not visible or markable and paint no overlay. A preserved explicit mutable decision still submits; an otherwise eligible mutable hidden occurrence submits as an effective explicit exclusion without entering session state. Immutable ancestry and expanded-exclusion omission still take precedence. | **CORRECTED** |
| INV-5.8 | **One shared visibility definition, separate disposition.** Live marking and silent highlighting use the same visibility result and never paint hidden geometry. Payload evaluation consumes that same result but intentionally maps hidden otherwise-mutable occurrences to effective exclusions as defined by INV-5.7; UI paintability and payload coverage must not be conflated. | **CORRECTED** |
...
| INV-5.14 | **Consent UI is non-interactive but payload-covered.** Suppressed page-authored consent DOM remains in sanitized HTML and paints no marking overlay. Its otherwise-mutable hidden occurrences follow INV-5.7 and submit effective explicit exclusions unless immutable or excluded ancestry already supplies coverage; those effective rows never enter session state. | **CORRECTED** |
...
| INV-5.16 | **The AI retains no property corpus.** Every run sends the complete current candidate-page corpus in one self-contained request. A returned session id and the extension's continuation record are temporary status/result polling state for that active marking session only; they are retired and generation-fenced at terminal session edges and provide no data to a later run. | **CORRECTED** |
...
| INV-6.3 | **AI-fresh-before-save gate and dismissible sessions.** After any marking change, Run AI must re-run before Save enables. Any post-AI edit drops back to dirty and re-requires a run. Clean marking may disable immediately; dirty disable/navigation requires approval and then completely discards the session, while cancellation preserves it exactly. | **CORRECTED** |
...
| INV-6.5 | **Save commit, then authoritative Load.** Save structurally carries exactly the current page plus domain-wide selectors; it never uploads a page map or the full corpus. A successful Save is only the remote commit fence. The client then performs one distinct `/load`, whose latest complete snapshot atomically replaces the background baseline before the session may finish in silent mode. The Save response is never adopted as local configuration authority, and a successful Load destroys the active mutable session without merging or retaining its rows, suggestions, draft, or pre-Load snapshot. | **CORRECTED** — full corpus is for AI, not Save; configuration authority is Load-only. |
...
| INV-6.7 | **Page-boundary navigation/reload terminates marking.** Full-document navigation/reload and same-document path/query changes synchronously guard a dirty session, then disable and discard it only on approval. Cancellation preserves URL and session exactly. Fragment-only movement is not a page boundary. No dismissed draft survives outside active marking. | **CORRECTED** |

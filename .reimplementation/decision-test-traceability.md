...
| D-07 | P3 | `tests/src/content/marking/dom-bridge.test.ts::coalesces window, visual viewport, and root resize into one silent geometry commit`; `tests/src/content/marking/dom-bridge.test.ts::keeps the marking resize deadline ahead of induced scroll and duplicate observers`; `tests/src/content/marking/dom-bridge.test.ts::drops net-zero responsive churn, branch-refreshes class, and rebuilds role`; `tests/src/content/marking/dom-bridge.test.ts::drops freeze-owned style locks but retains page-authored style changes`; `tests/src/content/marking/dom-bridge.test.ts::retains silent nodes while one quiet scroll transaction repositions them` | P11 scroll/resize alignment and Long Task comparison |
...
| N-14 | P25 | `tests/src/popup/sections/preview-row-list.test.ts::keeps production readable and debug diagnostics section-local` | P25 headed Preview keyboard/focus and two-way emphasis matrix |

...
| D-07 | P3 | `tests/src/content/marking/dom-bridge.test.ts::coalesces window, visual viewport, and root resize into one silent geometry commit`; `tests/src/content/marking/dom-bridge.test.ts::retains silent nodes while one quiet scroll transaction repositions them` | P11 scroll/resize alignment and Long Task comparison |

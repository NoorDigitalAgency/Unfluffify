...
| I-11 | P5 | `tests/src/popup/entrypoint.test.ts::commits one first configuration then adopts only the distinct Load authority`; `tests/src/background/startup.test.ts::treats accepted Save as commit-only and fresh Load as the sole local authority`; `tests/src/background/property-authority.test.ts::keeps local posture until the mandatory post-save Load replaces it` | P11 singular Save followed by distinct remote Load |
...
| I-33 | P4 | `tests/src/background/emulation-policy.test.ts::establishes the fixed crawler posture once and leaves held authority untouched`; `tests/src/background/render-emulation-runtime.test.ts::suspends an active posture behind the guard, retains desire, and resumes through apply`; `tests/src/background/render-emulation-runtime.test.ts::records an ownerless default directly as suspended without touching presentation or CDP`; `tests/src/background/render-emulation-runtime.test.ts::does not reassert a persisted suspended posture on cold startup or refit`; `tests/src/background/render-emulation-runtime.test.ts::retains the opaque guard for normal resume when an owner returns after durable suspension`; `tests/src/messaging/emulation-compositor-owner-port.test.ts::emits only first-owner and last-owner edges when multiple panels bind the same tab`; `tests/src/background/render-inspection-startup.test.ts::rejects a start whose panel owner closes during context authorization`; `tests/c4-content-entrypoint.test.ts::adopts ownerless emulation suspension from page context before accepting marking` | `EL03-R13-AC-01` through `EL03-R13-AC-04`: exact-pushed open/close/reopen compositor lifecycle, owned navigation/detach/bounds self-heal, and full-screen fit |
...
| D-07 | P3 | `tests/src/content/marking/dom-bridge.test.ts::coalesces window, visual viewport, and root resize into one silent geometry commit`; `tests/src/content/marking/dom-bridge.test.ts::keeps the marking resize deadline ahead of induced scroll and duplicate observers`; `tests/src/content/marking/dom-bridge.test.ts::does not scan marking targets during a silent-only geometry transaction`; `tests/src/content/marking/dom-bridge.test.ts::starts the marking repaint at 230 ms when silent highlights are also armed`; `tests/src/content/marking/dom-bridge.test.ts::bounds silent viewport geometry while retaining offscreen node identity`; `tests/src/content/marking/dom-bridge.test.ts::drops net-zero responsive churn, branch-refreshes class, and rebuilds role`; `tests/src/content/marking/dom-bridge.test.ts::drops freeze-owned style locks but retains page-authored style changes`; `tests/src/content/marking/dom-bridge.test.ts::retains silent nodes while one quiet scroll transaction repositions them`; `tests/src/content/marking/root-authority.test.ts::retires every connected superseded root and leaves detached history alone`; `tests/p23-frozen-presentation-contract.test.ts::requires physical hover and scroll under a permanently starved page rAF`; `tests/p25-workflow-probes.test.ts::accepts the exact visual viewport while preserving the native scrollbar gutter`; `tests/p25-live-probes.test.ts::selects the painted current root and the newest root when paint counts tie` | P11 scroll/resize alignment and Long Task comparison |
| D-08 | P3 | `tests/c4-content-entrypoint.test.ts::uses one transaction for marking and silent-selector entrypoints and disposes overlays`; `tests/src/content/input-firewall.test.ts::covers the complete pointer, mouse hover, capture, and cancellation matrix` | P18/P25 native right-click pass-through flow |
...
| N-01 | P13 | `tests/src/content/marking/dom-bridge.test.ts::keeps consent-suppressed page DOM in payload rows and capture without making it interactive`; `tests/src/content/marking/dom-bridge.test.ts::retains consent-hidden subtrees regardless of helper attribute quoting` | `ACCEPT-P13-CAPTURE-SANITIZER`; `ACCEPT-P13-CONSENT-LIFECYCLE` with retained payload-row evidence |
| N-02 | P15 | `tests/src/popup/entrypoint.test.ts::opens silent preview with silent origin`; `tests/c4-content-entrypoint.test.ts::registers typed preview rows and retires their hover and bridge across exit and A-to-B navigation`; `tests/src/content/interaction-shield.test.ts::waits through the native presentation boundary before applying a wheel fallback` | `ACCEPT-P15-FROZEN-SHIELD` |
| N-03 | P16 | `tests/src/background/render-inspection-runtime.test.ts::terminalizes and fully fails open before panel-owned debugger suspension` | `ACCEPT-P16-INSPECTION-LIFECYCLE`; `EL03-R13-AC-03` last-panel cancellation ordering |
...
| N-07 | P18 | `tests/src/ui/transient-surface-manager.test.ts::mutually excludes menus and closes only the outside top menu`; `tests/src/ui/transient-surface-manager.test.ts::dismisses a nested checklist confirmation before its parent`; `tests/src/ui/transient-surface-manager.test.ts::uses Preview only as an empty-stack fallback and requests each exit once`; `tests/c4-content-entrypoint.test.ts::uses one transaction for marking and silent-selector entrypoints and disposes overlays` | `ACCEPT-P18-TRANSIENT-ESCAPE`, including native right-click preservation |

...
| D-07 | P3 | `tests/src/content/marking/dom-bridge.test.ts::coalesces window, visual viewport, and root resize into one silent geometry commit`; `tests/src/content/marking/dom-bridge.test.ts::keeps the marking resize deadline ahead of induced scroll and duplicate observers`; `tests/src/content/marking/dom-bridge.test.ts::does not scan marking targets during a silent-only geometry transaction`; `tests/src/content/marking/dom-bridge.test.ts::starts the marking repaint at 230 ms when silent highlights are also armed`; `tests/src/content/marking/dom-bridge.test.ts::drops net-zero responsive churn, branch-refreshes class, and rebuilds role`; `tests/src/content/marking/dom-bridge.test.ts::drops freeze-owned style locks but retains page-authored style changes`; `tests/src/content/marking/dom-bridge.test.ts::retains silent nodes while one quiet scroll transaction repositions them`; `tests/src/content/marking/root-authority.test.ts::retires every connected superseded root and leaves detached history alone`; `tests/p23-frozen-presentation-contract.test.ts::requires physical hover and scroll under a permanently starved page rAF`; `tests/p25-workflow-probes.test.ts::accepts the exact visual viewport while preserving the native scrollbar gutter`; `tests/p25-live-probes.test.ts::selects the painted current root and the newest root when paint counts tie` | P11 scroll/resize alignment and Long Task comparison |

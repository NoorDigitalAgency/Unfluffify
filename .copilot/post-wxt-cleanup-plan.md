...
- Runtime still has 1,884 tracked `@ts-expect-error` suppressions across:
...
- `src/popup.ts` is down to 332 tracked suppressions after typing the
  `refreshUi()` view-state projector against `uiModule.getViewState()` and
  removing the untyped `nextViewState` mutation cluster.

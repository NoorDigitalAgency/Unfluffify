...
- Plain click toggles the individual mutable state; Shift changes breadth and may widen to an eligible ancestor; Alt toggles individual explicit inclusion and wins over Shift. Alt may atomically move an explicit inclusion from a mixed-text parent to its child. Expanded-exclusion ownership/rehydration and explicit-inclusion exceptions are defined in `MARKING_AND_HIGHLIGHTING_LOGIC.md`.
...
- Marking mode uses `Alt` for explicit include, `Shift` for parent selection, and hold-`Space` for temporary page UI interaction/pass-through. Meta/Ctrl have no marking semantics. The extension never intercepts `contextmenu`.

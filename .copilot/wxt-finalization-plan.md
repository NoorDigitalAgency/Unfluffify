...
- The latest `src/content-main.ts` zero-suppression checkpoint is complete: page-hotkey gating helpers, saved-entry draft checks, silent-highlighting reveal keys and lifecycle emission, selector-suppression accessors, silent overlay setup/hide helpers, and property-lock client/reconnect/error handlers now use concrete URL/config/DOM/lifecycle types instead of broad helper suppressions.
- That checkpoint also relaxed the coupled source-contract tests only enough to tolerate the new TypeScript annotations for `toggleEnabledFromPage(...)`, `markExtensionContextInvalidated(...)`, `handleBlockedPropertyLockInteraction(...)`, `setPropertyLockClientId(...)`, `getEffectiveAiSelectorSet(...)`, and `setSilentHighlightingPageMotionPaused(...)` while preserving the same wiring assertions.
- `src/content-main.ts` is now suppression-free. Full review/validation is green for that checkpoint (`code-review`: CLEAN, then `pnpm lint && pnpm check && pnpm test && pnpm build`).
- Current suppression count is 191 total; the remaining tracked files are `src/popup.ts` (111) and the exempt eval bridge pair (`src/common/page-motion-freeze-bridge.ts` 43, `src/common/page-motion-freeze-control.ts` 37).
- Next immediate step: attack the remaining `src/popup.ts` tail in a large batch; after that, only the intentionally exempt eval bridge pair should remain.

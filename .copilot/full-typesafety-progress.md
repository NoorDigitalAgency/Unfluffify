...
- [2026-06-17] Phase 2.5: fully typed `common/text.ts` (typed property-lock text formatter callbacks, removed final 14 directives); updated source-contract tolerance in `tests/property-lock.test.js`; gates + full suite + ratchets green; suppression total 2553.
- [2026-06-17] Phase 2.6: fully typed `popup/messages.ts` (typed tab/message helper params, render-mode inspection payload/options handling, and active-tab fallback arg; removed final 15 directives); updated source-contract tolerance in `tests/background-render-mode-inspection.test.js` and `tests/device-emulation-lifecycle.test.js`; gates + full suite + ratchets green; suppression total 2538.
- [2026-06-17] Phase 2.7: fully typed `popup/property-lock-ui.ts` (typed dependency and payload parameters across lock-state helpers, refresh/command options, removed final 17 directives); updated source-contract tolerance in `tests/popup-marking-refresh.test.js`; gates + full suite + ratchets green; suppression total 2521.

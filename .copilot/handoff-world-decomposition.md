...
Branch: `main`
Status: world decomposition complete; content follow-up D0-D2, E0-E2, and Track F F1-F21 complete and pushed per-slice.
...
This baseline includes F21 (`setPageSaveReconciliationPending`) extracted into
`content/page-save-reconciliation-pending-handler.js` with focused tests plus a
green full suite.
...
Start with Phase F22 in `.copilot/track-f-protected-content-plan.md`.

F22 is the recommended next slice because it keeps the input guard local in
`content-main.js` while moving the clear-and-refresh success path behind a
single handler.

Before editing F22, run:
...
2. full test suite passes (`906 pass / 0 fail` at the current baseline)

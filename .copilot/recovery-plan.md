...
- **Session save (revised per user directive — post-save flow):** save uploads all local marked pages for the property (`includeAllLocalPageMarkings: true`); AI must be re-run after local marking changes before save is enabled (`doesSessionRequireAiRun`, `sessionRequiresAiRun`). On save completion the backend **returns the current payload**, and we **update local data from that returned payload** (so the AI payload for other pages stays current). Then:
  1. The current page's marking **rendering resets from scratch to the defaults → CSS/AI selector influence baseline** (the just-saved session explicit deltas are dropped from the overlay; saved markings are not replayed as a precedence layer).
  2. The mode **switches from marking mode to highlighting (silent-highlight) mode**, and the **content preview popup is shown**.
  3. The user **stays in highlighting mode** until they click **Enable Marking** again, which **re-enters marking mode starting from scratch at defaults → CSS/AI selector influence** (no carried-over session explicit deltas).
  - Reconciliation is cleared only after the forced backend reload confirms the page; exiting marking is blocked until saved or discarded.
...
### Unit C — Background lifecycle/spinner broker + busy-curtain reconciliation + reload restoration + activation order (the corrupted zone — rebuild cleanly)
**Genuine behavior**
- Cross-world lifecycle/spinner contract is centralized in `common/world-messaging-contract.js`. New lifecycle/spinner interactions must use the exported message types, lifecycle kinds/phases, content modes, and popup-state port helper; do not mint ad hoc `chrome.runtime` message strings in popup/content/background.
- Spinner queue current state is owned by the background broker, not by per-tab `chrome.storage` replay. Popup calls background `ufSpinnerSet`/`ufSpinnerRemove`/`ufSpinnerClear`, subscribes to `ufPopupState:<tabId>`, and mirrors `getUfBackgroundState`. `popupSpinnerQueue` remains a local mirror only. `navInspect` can persist in background across popup open/close; transient `runWithSpinner(null, ...)` keys are cleared when popup ports disconnect.
- Content emits `ufLifecycleEvent` for content-ready, marking activation, render-mode inspection, reveal/freeze progress, HTML capture, finish, and failure. Background stores the latest per-tab lifecycle operation with operation IDs and ignores stale terminal events from older operation IDs. Popup busy state mirrors this lifecycle current state first; polling is fallback/diagnostic only.

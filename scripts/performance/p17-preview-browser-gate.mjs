...
} from "./p17/contract.mjs";
import { renderFixturePage } from "./p17/fixture.mjs";

const scriptDirectory = dirname(fileURLToPath(import.meta.url));
const repositoryRoot = resolve(scriptDirectory, "../..");
const p17Directory = join(scriptDirectory, "p17");
const outputDirectory = join(repositoryRoot, "output/playwright/p17-preview");
...
const playwrightCli = resolve(process.env.P17_PLAYWRIGHT_CLI ?? join(p17Directory, "playwright-cli.sh"));
const smoke = process.argv.includes("--smoke");
const startedAt = new Date();
const artifactTimestamp = startedAt.toISOString().replaceAll(":", "-").replace(".", "-");
const retainedArtifactPath = smoke
  ? join("/tmp", `unfluffify-p17-preview-smoke-${process.pid}.json`)
...
async function sourceIdentity() {
  ...
  const status = rawStatus
    .split("\n")
    .filter(Boolean)
    .filter((line) => !line.slice(3).startsWith("output/playwright/p17-preview/"));
  ...
  const manifestPaths = (await git(
    ...
    "package.json",
    "pnpm-lock.yaml",
    "scripts/performance/p17-preview-browser-gate.mjs",
    "scripts/performance/p17",
    "tests/p17-browser-preview-contract.test.ts",
  )).split("\n").filter(Boolean);
...
async function buildRuntimeBundles() {
  const common = {
    ...
    entryPoints: ["scripts/performance/p17/runtime.tsx"],
    bundle: true,
    format: "iife",
    platform: "browser",
    target: ["chrome120"],
...
async function startFixtureServer() {
  ...
  const server = createServer(async (request, response) => {
    try {
      ...
      if (request.method === "GET" && url.pathname === "/") {
        ...
        response.end("<!doctype html><title>P17 gate</title><p>P17 canonical preview controller</p>");
        return;
      }
      ...
      if (request.method === "POST" && url.pathname === "/results") {
        ...
        for await (const chunk of request) {
          bytes += chunk.length;
          if (bytes > 10_000_000) throw new Error("P17 browser payload exceeded 10 MB");
          chunks.push(chunk);
        }
  ...
  if (!address || typeof address === "string") throw new Error("Unable to determine P17 fixture port");
  return {
...
async function playwrightVersion() {
  ...
  if (actual !== PLAYWRIGHT_CLI_VERSION) {
    throw new Error(`P17 requires @playwright/cli ${PLAYWRIGHT_CLI_VERSION}, received ${actual}`);
  }
  return { expected: PLAYWRIGHT_CLI_VERSION, actual, raw: result.stdout };
}

async function runPlaywrightCli(baseUrl) {
  const session = `p17-gate-${process.pid}`;
  ...
  try {
    ...
    return await run(playwrightCli, [
      "run-code",
      `--filename=${join(p17Directory, "playwright-controller.js")}`,
...
const report = {
  ...
  contract: {
    ...
    catalog,
    viewport: VIEWPORT,
    entrypointRaceAuthorities: ENTRYPOINT_RACE_AUTHORITIES,
  },
  browser: {
...
const failurePath = join("/tmp", `unfluffify-p17-preview-failure-${process.pid}.json`);

...
test("explicit marking handler stores page-scoped selector suppression when explicit marks are removed", () => {
  const source = readFileSync(new URL("../src/content/explicit-marking-handler.ts", import.meta.url), "utf8");
...
test("background owns the live-page GraphQL transport handlers", () => {
  ...
  const livePageClientSource = readFileSync(new URL("../src/background/live-page-client.ts", import.meta.url), "utf8");
...
test("marking mode stores default ancestors as unexcluded when descendants are marked", () => {
  const coreSource = readFileSync(new URL("../src/content/core.ts", import.meta.url), "utf8");
  const handlerSource = readFileSync(new URL("../src/content/explicit-marking-handler.ts", import.meta.url), "utf8");
...
test("explicit marking toggles cache XPath element resolution per operation", () => {
  const contentSource = readFileSync(new URL("../src/content/runtime-message-handler.ts", import.meta.url), "utf8");
  const handlerSource = readFileSync(new URL("../src/content/explicit-marking-handler.ts", import.meta.url), "utf8");
...
test("marking contract is locked across docs, memory, plan, and README", () => {
  ...
  const constantsSource = readFileSync(
    new URL("../src/common/constants.ts", import.meta.url),
    "utf8"
  );
  ...
  assert.doesNotMatch(readmeSource, /node --test/);
  assert.doesNotMatch(readmeSource, /deno test/);

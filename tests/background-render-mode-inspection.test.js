...
test("background executeRenderModeInspection orchestrates reload, consent hide, capture, and end", () => {
  ...
  assert.match(backgroundSource, /async function captureRenderModeHtmlWithDebugger\(tabId(?:\s*:\s*[^)]+)?\)(?:\s*:\s*[^{]+)? \{[\s\S]*?"DOM\.getDocument"[\s\S]*?"DOM\.getOuterHTML"[\s\S]*?fetchStaticPageHtmlForBackground\(pageUrl\)/);
  assert.match(
    commandBlock,
...
test("background restores no-JS render-mode holds after central tab inactivity", () => {
  ...
  assert.match(backgroundSource, /async function updateRenderModeNoJsInactivityWatch\(tabId(?:\s*:\s*[^)]+)?\)(?:\s*:\s*[^{]+)? \{[\s\S]*?isTabActiveInFocusedWindow\(normalizedTabId\)[\s\S]*?tabInactivityObserver\.scheduleInactive\(normalizedTabId, \{[\s\S]*?scope: RENDER_MODE_NO_JS_INACTIVITY_SCOPE/);
  assert.match(backgroundSource, /async function restoreRenderModeJavaScriptAfterNoJsInactivity\(\s*tabId(?:\s*:\s*[^,)]+)?,?\s*\)(?:\s*:\s*[^{]+)? \{[\s\S]*?isTabActiveInFocusedWindow\(normalizedTabId\)[\s\S]*?clearRenderModeNoJsHeld\(normalizedTabId\)[\s\S]*?utils\.reloadPageWithJavaScriptControl\(normalizedTabId, false\)[\s\S]*?utils\.detachDebugger\(normalizedTabId\)/);
  assert.match(backgroundSource, /async function restoreRenderModeJavaScriptAfterNoJsInactivity\(\s*tabId(?:\s*:\s*[^,)]+)?,?\s*\)(?:\s*:\s*[^{]+)? \{[\s\S]*?brain\.recordRenderModeNoJsHold\(normalizedTabId, \{[\s\S]*?held: false,[\s\S]*?javaScriptDisabled: false[\s\S]*?\}, "render-mode:no-js-inactivity:cleared"\)/);
  assert.match(backgroundSource, /async function restoreRenderModeJavaScriptAfterNoJsInactivity\(\s*tabId(?:\s*:\s*[^,)]+)?,?\s*\)(?:\s*:\s*[^{]+)? \{[\s\S]*?setPageJavaScriptExecutionDisabled\(normalizedTabId, false\)[\s\S]*?setRenderModeNoJsHeld\(normalizedTabId, true\)[\s\S]*?updateRenderModeNoJsInactivityWatch\(normalizedTabId\)/);
  assert.match(backgroundSource, /async function restoreRenderModeJavaScriptAfterNoJsInactivity\(\s*tabId(?:\s*:\s*[^,)]+)?,?\s*\)(?:\s*:\s*[^{]+)? \{[\s\S]*?setRenderModeNoJsHeld\(normalizedTabId, true\)\.catch\(\(\) => null\);[\s\S]*?brain\.recordRenderModeNoJsHold\(normalizedTabId, \{[\s\S]*?held: true,[\s\S]*?javaScriptDisabled: true[\s\S]*?\}, "render-mode:no-js-inactivity:restored-hold"\)/);

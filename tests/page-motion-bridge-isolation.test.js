...
test("page motion control function has no persistent page message listener", () => {
  const source = readFileSync(new URL("../common/page-motion-freeze-control.ts", import.meta.url), "utf8");
  const serialized = Function.prototype.toString.call(runPageMotionFreezeControl);

  assert.equal(existsSync(new URL("../common/page-motion-freeze.ts", import.meta.url)), false);

...
describe("local storage is only for an unconfigured property", () => {
  it("stores an operator's render mode while the backend has no configuration", async () => {
    const svc = services();
    await svc.property.applyBackendLoad(SITE_ID, { status: "not_found" });

    await expect(svc.property.rememberRenderMode(SITE_ID, "static")).resolves.toEqual({ stored: true });

    const stored = await svc.repos.localPropertyRepo.load(SITE_ID);
  ...
  it("refuses to store one once the backend has a configuration", async () => {
    const svc = services();
    await svc.property.applyBackendLoad(SITE_ID, { status: "ok", config: BACKEND_CONFIG });

    await expect(svc.property.rememberRenderMode(SITE_ID, "static"))
...
describe("the backend is the single source of truth", () => {
  it("drops the local render mode when the backend returns a configuration", async () => {
    const svc = services();
    await svc.property.applyBackendLoad(SITE_ID, { status: "not_found" });
    await svc.property.rememberRenderMode(SITE_ID, "static");

    const applied = await svc.property.applyBackendLoad(SITE_ID, { status: "ok", config: BACKEND_CONFIG });
  ...
  it("keeps only the render mode when the backend has nothing stored", async () => {
    ...
    const applied = await svc.property.applyBackendLoad(SITE_ID, { status: "not_found" });
    ...
    const cachedConfig = await svc.repos.configRepo.load(SITE_ID);
  ...
  it("clears the cached configuration on both kinds of answer", async () => {
    for (const outcome of [
      ...
      await svc.property.applyBackendLoad(SITE_ID, outcome);

      const cached = await svc.repos.configRepo.load(SITE_ID);
      expect(cached.ok && cached.value, `status ${outcome.status}`).toBeNull();
    }
  });
  ...
  it("changes nothing when the backend never answered", async () => {
    for (const status of ["auth_error", "error"] as const) {
      ...
      const applied = await svc.property.applyBackendLoad(SITE_ID, { status });
  ...
  it("drops the local render mode once a save puts it on the backend", async () => {
    ...
    await svc.property.applyBackendSave(SITE_ID);

    const stored = await svc.repos.localPropertyRepo.load(SITE_ID);
    expect(stored.ok && stored.value).toMatchObject({ backendConfigPresent: true });
    expect(stored.ok ? stored.value?.renderMode : "read-failed").toBeUndefined();
    // And a later choice is refused, since the backend now owns it.
    await expect(svc.property.rememberRenderMode(SITE_ID, "rendered"))
      .resolves.toEqual({ stored: false, reason: "backend-config-present" });
  });
  ...
  it("keeps properties independent", async () => {
    ...
    await svc.property.applyBackendLoad(SITE_ID, { status: "not_found" });
    await svc.property.rememberRenderMode(SITE_ID, "static");
    await svc.property.applyBackendLoad(9999, { status: "ok", config: { ...BACKEND_CONFIG, siteId: 9999 } });

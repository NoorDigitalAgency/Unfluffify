...
5. In two other terminals, smoke the bus:
   ...
   ```bash
   pnpm orchestrate:mock-client -- --role director --side A --note "director online"
   pnpm orchestrate:mock-client -- --role follower --side B --note "follower online"
   ```
...
```bash
pnpm orchestrate:runner -- --role follower --side B --bus-host 127.0.0.1 --bus-port 8765
```
...
```bash
pnpm orchestrate:setup-auth -- --role director --side A --account A --profile-dir orchestration/profiles/director
pnpm orchestrate:setup-auth -- --role follower --side B --account B --profile-dir orchestration/profiles/follower
```
...
```bash
xvfb-run -a -s "-screen 0 1280x1024x24" pnpm orchestrate:setup-auth -- --role director --side A --account A --profile-dir orchestration/profiles/director
```
...
```bash
xvfb-run -a -s "-screen 0 1280x1024x24" pnpm orchestrate:property-lock -- --property-url https://www.bonliva.no/ --cross-property-url https://prowork.se/ --director-profile-dir orchestration/profiles/director --follower-profile-dir orchestration/profiles/follower
```
